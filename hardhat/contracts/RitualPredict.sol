...
contract RitualPredict {
    ...
    function createMarket(
        ...
        // The point of the contract: the market books its own resolution here, in the
        // same transaction that creates it. There is no way to end up with a market
        // that nobody ever comes back to settle.
    ...
    function onScheduledResolve(
        ...
        // The only revert in this function, per the contract note above.
        ...
        if (m.state == MarketState.Resolved || m.state == MarketState.Invalid)
            return; // already final: a leftover booked execution is harmless
        ...
        if (!ok) {
            // Never read as NO: a read that did not happen says nothing about the
            // answer. Retry, and refund everyone if the attempts run out.
        ...
        // Pari-mutuel has no denominator when nobody backed the winning answer, so
        // record what was observed and then let everyone refund.
        ...
        // The attempts still booked are no longer needed. In try/catch so a refusal on
        // the Scheduler side cannot undo a resolution that has already succeeded.
        if (m.scheduleId != 0) {
    ...
    function _readOracle(
        ...
        // 13-field HTTPCallRequest. The order is load-bearing: the executor decodes
        // positionally, so one transposed field silently changes the request.
        bytes memory request = abi.encode(
            executor,
            new bytes[](0), // encryptedSecrets: the oracle URL is public
            HTTP_TTL_BLOCKS,
            new bytes[](0), // secretSignatures
            bytes(""), // userPublicKey: a plaintext response is fine
            m.oracleUrl,
            RitualChain.HTTP_GET,
            new string[](0), // headerKeys
            new string[](0), // headerValues
            bytes(""), // body: a GET carries none
        ...
        // Through an external `try` on purpose: malformed bytes then arrive as a caught
        // failure instead of reverting the whole scheduled execution.
        try this.decodeHttpResponse(raw) returns (
            ...
            if (status != 200) return (false, 0, "http status was not 200");

            (bool parsed, uint256 observed) = _jqUint(m.jsonPath, string(body));
            if (!parsed) return (false, 0, "jq did not yield a uint256");

            return (true, observed, "");
        } catch {
            return (false, 0, "http response could not be decoded");
        }
    }
    ...
    function _pickExecutor(
        ...
        // The seed deliberately avoids block.number and blockhash. A short-running async
        // call is simulated once to build the commitment and then replayed with the
        // settled output injected; if the executor differed between those two passes the
        // request bytes would differ too. executionIndex still re-rolls the choice on
        // every retry, so one unhealthy executor cannot sink a market.
        uint256 seed = uint256(
            keccak256(abi.encode(address(this), marketId, executionIndex))
        );
        ...
        returns (address teeAddress, bool found) {
            ...
            // An unreachable registry is a failed attempt, never a NO.
            return address(0);
        }
    }
    ...
    function _scheduleResolution(
        ...
        // The Scheduler overwrites calldata bytes 4-35 with the real executionIndex, so
        // the first argument must be a uint256 and is encoded as a placeholder here.
        ...
        // Authorise a ceiling rather than today's basefee alone: these executions fire
        // minutes from now, and an execution priced out is an execution skipped.
        uint256 maxFeePerGas = block.basefee * 2;
        if (maxFeePerGas < MIN_MAX_FEE_PER_GAS)
            maxFeePerGas = MIN_MAX_FEE_PER_GAS;

        // All MAX_ATTEMPTS retries are booked in this one call: retrying is the
        // Scheduler's own mechanism, so the contract never re-schedules itself later.
        // frequency * numCalls = 600, well under the Scheduler's MAX_LIFESPAN.
        callId = IScheduler(RitualChain.SCHEDULER).schedule(
            ...
            0, // value: the callback moves no native funds
